...
public class ZAandroid {
     ...
     *
     * @param fd
     * @param nDeviceType  设备类型（0：有驱动 USB 设备， 1：串口设备， 2：无驱 UDISK 设备,3：有驱动 USB 设备+0x30， 4：串口设备+0x30， 5：无驱 UDISK 设备+0x30）；(0x30  4.0版本)
     ...
     * @param iCom         串口号（1-16）（USB 设备和无驱 UDISK 设备该参数为 0）；(0-7   "/dev/ttySx",  10-17   "/dev/ttyMTx"  (x为0-7),
     *                     20-27   "/dev/ttysWKx"  (x为0-7),30-37   "/dev/ttyHSLx"  (x为0-7)),	40-47   "/dev/ttyMAXx"  (x为0-7)),
     *                     50-57  "/dev/ttyAMAx"  (x为0-7)),	60-67
     * @param iBaud        波特率（9600-57600）（USB 设备和无驱 UDISK 设备该参数为 0）波特率=iBaud*9600；
     * @param nPackageSize 通讯包大小（默认:2）
     * @param iDevNum      通讯端口号(默认 0)
     * @return 0 为成功，其它返回值请参考错误返回码
     */
     ...
     * 获取图像函数
     *
     * @param imagesize （0:256x288 1:256x360）  nAddr： 0xffffffff
     * @return 0 为成功，其它返回值请参考错误返回码
     */
     ...
     * 生成特征码函数
     * @param nAddr nAddr： 0xffffffff ；
     * @param iBufferID  0x01、 0x02(电容/光学) 0x01、 0x02、 0x03、 0x04(刮擦)
     * @return 0 为成功，其它返回值请参考错误返回码
     */
    public native int ZAZGenChar(int nAddr, int iBufferID);

    /**
     * 精确比对函数（比对 CharBufferA 与 CharBufferB）
     * @param nAddr 0xffffffff ；
     * @param iScore  iscore[0] 比对后的分数值
     * @return  0 为成功，其它返回值请参考错误返回码
     */
    ...
    /**
     * 搜索比对函数（以 CharBufferA 或 CharBufferB 中的特征文件搜索整个或部分指纹库）
     * @param nAddr 0xffffffff ；
     * @param iBufferID 0x01、 0x02
     * @param iStartPage 起始 ID；
     * @param iPageNum 结束 ID；
     * @param iMbAddress iMbAddress[0]搜索成功后模板 ID 号；
     *   iscore：                默认参数（NULL）
     * @return 0 为成功，其它返回值请参考错误返回码
     */
    ...
    /**
     * 合成模板函数(将 CharBufferA 与 CharBufferB 中的特征文件合并生成模板存于 ModelBuffer)
     * @param nAddr 0xffffffff ；
     * @return 0 为成功，其它返回值请参考错误返回码
     */
     ...
     * @param nAddr 0xffffffff ；
     * @param iBufferID 0x01、 0x02
     * @param iPageID  模板存储到指纹库中的 ID 号
     * @return  0 为成功，其它返回值请参考错误返回码
     */
    public native int ZAZStoreChar(int nAddr, int iBufferID, int iPageID);

    /**
     * 读出模板函数(从 flash 指纹库中读取一个模板到 ModelBuffer)
     * @param nAddr 0xffffffff ；
     * @param iBufferID 从指纹库中读出的模板所存放的特征缓冲区(0x01、 0x02)；
     * @param iPageID 在指纹库中将要读出的指纹模板 ID 号
     * @return 0 为成功，其它返回值请参考错误返回码
     */
