    ...
    private Runnable getFpTasks = new Runnable() {
        public void run() {
            ...
            if (nRet == 0) {
                ...
                data.setInfoMsg(mContext.getString(R.string.receive_erro));
                callBack.callBackInfo(data);
                removeCallbacks();
            } else {
                data.setInfoMsg(mContext.getString(R.string.communication_erro));
                callBack.callBackInfo(data);
                removeCallbacks();
                return;
            }
        ...
        public void run() {
            ...
            if (nRet == 0) {
                ...
                if (nRet == a6.PS_OK) {
                    ...
                    data.setInfoMsg(mContext.getString(R.string.template_bad));
            ...
            if (nRet == 0) {
                ...
                if (nRet == a6.PS_OK) {
                    ...
                    if (fpcharbuf > 2) {
                        ...
                        if (nRet != a6.PS_OK) {
                            data.setInfoMsg(mContext.getString(R.string.RegModulefail_str));
                            callBack.callBackInfo(data);
                        } else {
                            ...
                            if (nRet == a6.PS_OK) {
                                ...
                                data.setInfoMsg("ID:" + iPageID);
                                callBack.callBackInfo(data);
//                                handler.sendMessage(handler.obtainMessage(0, "合成模板 ID：" + iPageID));
                                iPageID++;
                            } else {
                                data.setInfoMsg(mContext.getString(R.string.enrollment_fail));
                                callBack.callBackInfo(data);
                            }
                        }
                    } else {
                        data.setInfoMsg(mContext.getString(R.string.get_image_again));
                        callBack.callBackInfo(data);
            ...
            if (nRet == 0) {
                ...
                if (nRet == a6.PS_OK) {
                    ...
                    if (nRet == a6.PS_OK) {
                        ...
                        if (nRet == a6.PS_OK) {
                            data.setInfoMsg("ID:" + id_iscore[0] + "\nComparisonNum：" + iScore[0]);
                            callBack.callBackInfo(data);
                        } else {
                            data.setInfoMsg("Not ComparisonNum：" + iScore[0]);
                            callBack.callBackInfo(data);
                        }
                    } else {
                        data.setInfoMsg(mContext.getString(R.string.search_fail));
                        callBack.callBackInfo(data);
                    }
                } else {
                    data.setInfoMsg(mContext.getString(R.string.template_bad));
        ...
        public void run()// 运行该服务执行此函数
        {
            ...
            if (nRet == 0) {
                ...
                if (nRet == a6.PS_OK) {
                    nRet = a6.ZAZHighSpeedSearch(DEV_ADDR, 1, 0, 1000, id_iscore);
                    if (nRet == a6.PS_OK) {
                        data.setInfoMsg("Search ID:" + id_iscore[0]);
                        callBack.callBackInfo(data);
                    } else {
                        data.setInfoMsg(mContext.getString(R.string.search_failed));
                        callBack.callBackInfo(data);
                    }
